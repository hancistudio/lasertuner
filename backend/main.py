...
ðŸŽ¯ FIREBASE-FIRST STRATEGY: Model only in Firebase Storage
"""
import sys
import io
...
# âœ… GLOBAL VARIABLES for Transfer Learning
transfer_model = None
...
class PredictionRequest(BaseModel):
    ...
    def validate_material(cls, v):
        """âœ… AppConfig uyumlu malzeme validasyonu"""
        valid_materials = {
            'ahÅŸap', 'ahsap', 'wood', 'kontrplak', 'plywood', 'mdf', 'balsa',
            'bambu', 'bamboo', 'kayÄ±n', 'kayin', 'beech', 'meÅŸe', 'mese', 'oak',
            'ceviz', 'walnut', 'akÃ§aaÄŸaÃ§', 'akcaagac', 'maple', 'huÅŸ', 'hus', 
            'birch', 'Ã§am', 'cam', 'pine', 'ladin', 'spruce', 'fir',
            'deri', 'leather', 'karton', 'cardboard', 'kaÄŸÄ±t', 'kagit', 'paper', 
            'kumaÅŸ', 'kumas', 'fabric', 'keÃ§e', 'kece', 'felt', 'mantar', 'cork', 
            'akrilik', 'acrylic', 'plexiglass', 'pleksiglas', 'pmma',
            'lastik', 'rubber', 'kÃ¶pÃ¼k', 'kopuk', 'foam', 'anodize_aluminyum',
        ...
        logger.warning(f"âš ï¸ Unknown material: {v}, allowing for flexibility")
        return v
...
async def predict(request: PredictionRequest):
    """
    ðŸ¤– TRANSFER LEARNING PREDICTION ENDPOINT
    ...
    try:
        logger.info(
            f"ðŸ”¥ Prediction request: {request.machineBrand} {request.laserPower}W, "
            f"{request.materialType} {request.materialThickness}mm, processes: {request.processes}"
        )
        
        warnings = []
        if request.materialThickness > 8:
            warnings.append(f"âš ï¸ {request.materialThickness}mm kalÄ±nlÄ±k zorlu olabilir")
        if request.materialThickness > 5 and request.laserPower < 20:
            warnings.append(f"âš ï¸ {request.laserPower}W gÃ¼Ã§ dÃ¼ÅŸÃ¼k olabilir")
        ...
        if transfer_model and TF_AVAILABLE and feature_encoder and transfer_model.is_trained:
            try:
                logger.info("ðŸ¤– Using trained transfer learning model...")
                ...
                for process_type in request.processes:
                    ...
                    logger.info(
                        f"âœ… {process_type}: TL â†’ power={pred['power']:.1f}%, "
                        f"speed={pred['speed']:.0f}mm/min, passes={pred['passes']}"
                    )
                
                data_source = "transfer_learning"
                confidence_score = 0.85
                notes = "ðŸ¤– Transfer learning model (Firebase Storage'dan yÃ¼klendi)"
                
            except Exception as e:
                logger.warning(f"âš ï¸ Transfer learning prediction failed: {e}")
                predictions = {}
        elif transfer_model and not transfer_model.is_trained:
            logger.info("â„¹ï¸ Model exists but UNTRAINED, using static algorithm")
            predictions = {}
        
        # ===== FALLBACK: STATIC ALGORITHM =====
        if not predictions:
            if transfer_model and not transfer_model.is_trained:
                logger.info("âš™ï¸ Using static algorithm (model exists but untrained)")
            else:
                logger.info("âš™ï¸ Using static algorithm fallback")
            ...
            if transfer_model and not transfer_model.is_trained:
                notes = "âš™ï¸ Statik algoritma (Model Firebase'de yok veya eÄŸitilmemiÅŸ - 50+ doÄŸrulanmÄ±ÅŸ deney gerekli)"
            else:
                notes = "âš™ï¸ Statik algoritma (TL model yok veya baÅŸarÄ±sÄ±z)"
        ...
        logger.info(f"âœ… Prediction complete in {duration:.3f}s, source: {data_source}")
        
        return response
        
    except ValueError as e:
        logger.warning(f"âŒ Validation error: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))
    except Exception as e:
        logger.exception(f"âŒ Unexpected error: {str(e)}")
        raise HTTPException(status_code=500, detail="Tahmin hatasÄ±")
...
async def fine_tune_model():
    ...
    try:
        ...
        logger.info(f"âœ… Encoded {len(X)} training samples")
        ...
        logger.info("ðŸ“¤ Uploading fine-tuned model to Firebase Storage...")
        ...
        logger.info("âœ… Fine-tuning complete and uploaded to Firebase")
...
async def train_from_scratch():
    ...
    try:
        ...
        # 3. Create new model
        logger.info("ðŸ†• Creating fresh model architecture...")
        ...
        logger.info("ðŸ”„ Training model from scratch...")
        ...
        logger.info("ðŸ“¤ Uploading trained model to Firebase Storage...")
        success = storage_service.save_model_to_storage(local_model_path)
        ...
        logger.info("âœ… Training complete and uploaded to Firebase")
...
async def reload_model_from_firebase():
    ...
    try:
        ...
        logger.info("ðŸ“¥ Downloading model from Firebase Storage...")
        ...
        logger.info("âœ… Model reloaded from Firebase Storage")
...
async def startup_event():
    """
    ðŸš€ FIREBASE-FIRST MODEL STRATEGY
    ...
    logger.info("="*80)
    logger.info("ðŸš€ LaserTuner ML API v3.0 - FIREBASE-FIRST STRATEGY")
    logger.info("ðŸŽ¯ Model Source: Firebase Storage ONLY (no local fallback)")
    logger.info("="*80)
    logger.info(f"ðŸ¤– TensorFlow Available: {TF_AVAILABLE}")
    
    # 1. Initialize feature encoder
    try:
        feature_encoder = get_feature_encoder()
        logger.info("âœ… Feature encoder initialized")
    except Exception as e:
        logger.error(f"âŒ Feature encoder failed: {e}")
        feature_encoder = None
    ...
    if not firebase.is_available():
        logger.error("âŒ Firebase Firestore not available - CRITICAL")
    else:
        stats = firebase.get_statistics()
        logger.info(f"âœ… Firestore: {stats.get('verified_experiments', 0)} verified experiments")
    
    if not storage_service.is_available():
        logger.error("âŒ Firebase Storage not available - CRITICAL")
    else:
        logger.info("âœ… Firebase Storage connected")
    
    # 3. MODEL LOADING STRATEGY: Firebase Storage ONLY
    if not TF_AVAILABLE or not feature_encoder:
        logger.warning("âš ï¸ TensorFlow or encoder unavailable, using static algorithm")
        transfer_model = None
        logger.info("="*80)
        return
    ...
    if storage_service.is_available():
        logger.info("ðŸ” Checking Firebase Storage for model...")
        
        if storage_service.model_exists_in_storage():
            logger.info("ðŸ“¦ Model found in Firebase Storage, downloading...")
            
            # Download to temp location
            if storage_service.load_model_from_storage(local_model_path):
                transfer_model = get_transfer_model(local_model_path)
                model_loaded = True
                logger.info("âœ… Model loaded from Firebase Storage")
                
                # Show metadata
                metadata = storage_service.get_model_metadata()
                if metadata:
                    logger.info(f"   ðŸ“Š Size: {metadata['size_mb']:.2f} MB")
                    logger.info(f"   ðŸ“… Updated: {metadata['updated']}")
            else:
                logger.error("âŒ Failed to download model from Firebase Storage")
        else:
            logger.warning("âš ï¸ Model NOT found in Firebase Storage")
    else:
        logger.error("âŒ Firebase Storage not available")
    
    # ===== If no model in Firebase, create NEW and train =====
    if not model_loaded:
        logger.info("ðŸ†• No model in Firebase Storage, creating fresh model...")
        transfer_model = get_transfer_model()  # New untrained model
        logger.info("âœ… Fresh model architecture created (UNTRAINED)")
        ...
        if firebase.is_available():
            ...
            if verified_count >= 50:
                logger.info(f"ðŸ“Š Sufficient data ({verified_count} experiments), training now...")
                    ...
                    if len(training_data) >= 30:
                        ...
                        if storage_service.is_available():
                            ...
                            if storage_service.save_model_to_storage(local_model_path):
                                logger.info("âœ… Model uploaded to Firebase Storage")
                                logger.info("   ðŸŽ¯ Firebase is now the single source of truth")
                            else:
                                logger.error("âŒ Failed to upload to Firebase Storage")
                    else:
                        logger.warning(f"âš ï¸ Only {len(training_data)} samples, need 30+")
                
                except Exception as e:
                    logger.error(f"âŒ Training failed: {e}")
                    logger.exception("Full error:")
            else:
                logger.warning(f"âš ï¸ Insufficient data: {verified_count} experiments (need 50+)")
                logger.info("   â„¹ï¸ Model will use static algorithm until more data available")
    
    # 4. Online learning (optional)
    try:
        learner = get_online_learner()
        if learner.should_update():
            logger.info("ðŸ”„ Running online learning update...")
            learner.update_material_statistics()
    except Exception as e:
        logger.warning(f"âš ï¸ Online learning failed: {e}")
    
    logger.info("="*80)
    logger.info("ðŸŽ¯ STRATEGY: Firebase Storage is the single source of truth")
    logger.info(f"ðŸ“Š Model Status: {'TRAINED' if (transfer_model and transfer_model.is_trained) else 'UNTRAINED'}")
    logger.info(f"ðŸ“¦ Model in Firebase: {storage_service.model_exists_in_storage() if storage_service.is_available() else 'Unknown'}")
    logger.info("="*80)
...
async def shutdown_event():
    """Cleanup"""
    logger.info("ðŸ‘‹ LaserTuner ML API Shutting down...")
