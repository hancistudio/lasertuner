...
class MLService {
  ...
  Future<bool> checkHealth() async {
    try {
      print('ðŸ” Checking API health: $API_URL/health');
  ...
  Future<ApiHealthStatus?> getHealthStatus() async {
    try {
      print('ðŸ” Fetching detailed health status: $API_URL/health');
      ...
      print('âŒ Health status fetch failed: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> getModelStatus() async {
    try {
      print('ðŸ“Š Fetching model status from: $API_URL/test');
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        print('ðŸ“Š Model Status Data: $data');
        return data;
      }
      return null;
    } catch (e) {
      print('âŒ Model status fetch error: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> getStatistics() async {
    try {
      print('ðŸ“Š Fetching statistics from: $API_URL/statistics');
      ...
      print('âŒ Statistics fetch error: $e');
      return null;
    }
  }
  ...
  Future<PredictionResponse> getPrediction(PredictionRequest request) async {
    try {
      print('ðŸ“¤ Sending prediction request to: $API_URL/predict');
      print('ðŸ“¦ Request data: ${jsonEncode(request.toMap())}');
      ...
      final response = await http
          ...
          .timeout(
            ...
            onTimeout: () {
              print(
                'â±ï¸ Request timeout after ${requestTimeout.inSeconds}s',
              );
              throw Exception(
  ...
  Future<Map<String, dynamic>?> getModelStorageInfo() async {
    try {
      print('ðŸ—„ï¸ Fetching model storage info from: $API_URL/test');
      ...
      print('âŒ Model storage info fetch error: $e');
      return null;
    }
  }
  ...
  Future<Map<String, bool>> checkApiCapabilities() async {
    try {
      ...
      print('âŒ API capabilities check error: $e');
      return {
        'transfer_learning_enabled': false,
        'transfer_learning_trained': false,
        'firebase_firestore_connected': false,
  ...
  Future<Map<String, dynamic>> getSystemStatusReport() async {
    try {
      ...
      print('âŒ System status report error: $e');
      return {
        'timestamp': DateTime.now().toIso8601String(),
        'api_online': false,
        'error': e.toString(),
  ...
  PredictionResponse generateFallbackPrediction(PredictionRequest request) {
    ...
    List<String> warnings = []; // âœ… YENÄ°: Warnings ekle

    // âœ… YENÄ°: KalÄ±nlÄ±k uyarÄ±sÄ±
    if (thickness > 8) {
      warnings.add(
      ...
      warnings.add(
        'âš ï¸ ${thickness}mm kalÄ±nlÄ±k iÃ§in dikkatli yaklaÅŸÄ±n',
      );
    }

    // âœ… YENÄ°: GÃ¼Ã§ uyarÄ±sÄ±
    ...
    return PredictionResponse(
      ...
      dataPointsUsed: 0,
      dataSource:
          'fallback', // âœ… DeÄŸiÅŸtirildi: static_algorithm â†’ fallback
      warnings: warnings, // âœ… YENÄ°: Warnings ekle
    );
  }
